...
def allocate_memory(handle, size, allocation_type=None, protection_type=None):
    ...
    if not allocation_type:
        allocation_type = ressources.structure.MEMORY_STATE.MEM_COMMIT.value
    if not protection_type:
        protection_type = ressources.structure.MEMORY_PROTECTION.PAGE_EXECUTE_READWRITE.value
    ressources.kernel32.SetLastError(0)
    address = ressources.kernel32.VirtualAllocEx(handle, None, size, allocation_type, protection_type)
    return address
...
def free_memory(handle, address, free_type=None):
    ...
    """
    if not free_type:
        free_type = ressources.structure.MEMORY_STATE.MEM_RELEASE
    ressources.kernel32.SetLastError(0)
    ret = ressources.kernel32.VirtualFreeEx(handle, address, 0, free_type)
    return ret
...
def read_ctype(handle, address, ctype, *, get_py_value=True, raw_bytes=False):
    ...
    result = ressources.kernel32.ReadProcessMemory(
        handle,
...
def write_ctype(handle, address, ctype):
    ...
    result = ressources.kernel32.WriteProcessMemory(
        handle,
...
def virtual_query(handle, address):
    ...
    """
    mbi = ressources.structure.MEMORY_BASIC_INFORMATION()
    ressources.kernel32.SetLastError(0)
    ressources.kernel32.VirtualQueryEx(handle, address, ctypes.byref(mbi), ctypes.sizeof(mbi))
    error_code = ctypes.windll.kernel32.GetLastError()
    if error_code:
        ressources.kernel32.SetLastError(0)
        raise exception.WinAPIError(error_code)
